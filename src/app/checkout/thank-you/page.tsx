...
export default function ThankYouPage() {
	...
	const subtotal = items.reduce((sum, item) => {
		...
		return sum + product.basePrice * item.quantity + (item.insurancePriceInCents || 0);
	}, 0);
						...
						<Card className="border-0 shadow-md">
							<CardContent className="p-8">
								<div className="flex items-start gap-6">
									...
									<div>
										<h1 className="text-3xl font-bold text-gray-900">Thank you — your order is confirmed</h1>
										<p className="mt-2 text-gray-600">We've sent a confirmation email with your receipt and shipping information.</p>
						...
						<div className="mt-6 space-y-4">
							<Card className="shadow-sm">
								...
								<CardContent className="p-4">
									{items.length === 0 ? (
										<p className="text-sm text-gray-500">No items (cart was cleared).</p>
									) : (
										items.map((item) => {
											const product = getProductById(item.productId);
											...
											return (
												<div key={item.productId} className="flex items-center gap-4 py-3 border-b last:border-b-0">
													<div className="w-16 h-16 relative rounded-md bg-gray-100 overflow-hidden">
														<Image src={product.colors[0].imageUrl} alt={product.name} fill className="object-contain p-2" />
													</div>
													...
													<div className="text-right">
														<div className="font-medium">${((product.basePrice * item.quantity + (item.insurancePriceInCents || 0)) / 100).toFixed(2)}</div>
													</div>
												</div>
											);
										})
