...
export default function CheckoutPage() {
	...
	const calculateInsuranceTotal = () => {
		return items.reduce((total, item) => {
			...
			return (
				total + (insurancePrice ? (insurancePrice * item.quantity) / 100 : 0)
			);
		}, 0);
	};
						...
						<Card>
							...
							<CardContent className="space-y-6">
								{items.map((item) => {
									...
									const insurancePrice = getInsurancePrice(item.productId);
									const insurancePlanDuration = getInsurancePlanDuration(
										item.productId,
									);
									const itemSubtotal =
										(product.basePrice * item.quantity * 100) / 100;
									const itemInsuranceTotal = insurancePrice
										? (insurancePrice * item.quantity) / 100
										: 0;
									const planDurationLabel =
										insurancePlanDuration === "yearly" ? "1 Year" : "2 Years";
												...
												{hasItemInsurance ? (
													<div className="flex justify-between items-center">
														<span className="text-gray-700">
															Insurance ({planDurationLabel}): $
															{(itemInsuranceTotal / 100).toFixed(2)}
														</span>
														<Button
															variant="outline"
															size="sm"
